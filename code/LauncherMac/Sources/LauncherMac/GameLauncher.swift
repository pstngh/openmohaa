...
struct GameLauncher {
    ...
    static func launchBots(settings: LauncherSettings) {
        ...
        args.append(contentsOf: ["+set", "fraglimit", "0"])
        args.append(contentsOf: ["+set", "g_no_grenades", "1"])
        // Cheats are intentional for this private bot-training session, but
        // remain disabled for normal engine and multiplayer launches.
        args.append(contentsOf: ["+set", "thereisnomonkey", "1"])
        args.append(contentsOf: ["+set", "cheats", "1"])
