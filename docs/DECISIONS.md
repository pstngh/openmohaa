...
**Decision:** Keep always-on telemetry as complete schema-consistent triplets and rotate before the engine's signed 2 GiB file boundary. Each 1.5 GiB frame segment closes with `session_end`; recording continues in a timestamped directory with standard filenames, headers, metadata, and a new session. Service-start cleanup must remove the exact primary triplet and `telemetry/segments` together without creating an archival copy. Telemetry pulls are transient analysis inputs and are deleted after analysis.

**Rationale:** The `66af0cff` logger stopped at 2,153,632,622 frame bytes because the next file-length check overflowed its internal signed representation and misclassified the triplet. Segmentation prevents recurrence, preserves analysis boundaries, and honors the owner's policy that recording stays enabled while a service restart begins a clean capture and raw telemetry is not retained as backup data.

## D018 - Door and ladder completion must be geometric and bounded

**Decision:** After contact with a fully open door panel clears, carry only the captured through-door approach until the bot advances 128 units from the contact origin or three seconds elapse; keep the panel-end tangent contact-only and retain world-collision cancellation. After a grounded ladder's existing 64-unit exit owner releases, reject a real reattachment attempt and reclaim the same away direction until the bot advances 128 units from the detach origin or 2.5 seconds elapse. Neither completion may call `FindPath`, restore post-contact tangent carry, or alter ordinary wall placement.

**Rationale:** In the complete `849be809` sample, 13 of 14 long open-panel episodes had already selected a valid tangent before the strategic route pulled the bot back, and grounded ladder reacquisition clustered at 1,050 ms immediately after the one-second owner released. Stable local geometry already identifies the intended through-door and away-from-ladder directions; a bounded distance condition closes those ownership gaps without reviving rejected door repathing or conflating the separate wall-clearance problem.
