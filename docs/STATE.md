...
- Passing commit `6938284a` is deployed and fresh schema 7 telemetry is recording. Its first live sample already contains 31 `bot_route_turn_limited` and 11 `bot_open_door_panel_contact` events, proving both hooks execute. This is instrumentation/runtime confirmation only; owner visual, longer-run, objective, and multi-map acceptance are pending, and corridor placement plus base-path lean remain separate.
...
- The external service was verified active after deploying the independently checked artifact for passing commit `6938284a` on 2026-08-10. Its live binary pair matched that artifact, both intended UDP ports listened, and a fresh schema 7 triplet was recording; revalidate before later deployment because external state can change.
