...
- Passing commit `b094fd87` is deployed with human-like door command commitment and a fresh schema 7 capture. In the first autonomous 53 seconds, repeated same-door push contacts ended within about four seconds rather than the prior 60-240 seconds, but owner and longer multi-map acceptance remain pending. Existing strafe/lean behavior and the separate wall-hugging concern are unchanged.
...
- The external service was verified active after deploying the independently checked artifact for passing commit `b094fd87` on 2026-08-10. Its live binary pair matched that artifact and a fresh schema 7 triplet was recording; revalidate before later deployment because external state can change.
