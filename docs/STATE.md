...
Have the owner visually test deployed `6e472321` on `obj_team2`, annotating wall/railing proximity, bounce, hesitation, doors, and ladder attach/detach behavior, then exercise `obj_team4` and an FFA/practice map. Pull the fresh schema 7 triplet and compare the same strict one-sided-clearance and long-episode measures against `6938284a`. Accept the corridor inset only if it reduces wall-side travel without new stalls, doorway failures, objective regressions, or loss of combat/roaming personality; keep ladder recovery and context-aware base-path lean separate.
