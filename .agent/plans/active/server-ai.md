...
- The external service was verified active on 2026-08-10 with the independently checked artifact for passing commit `1c1ed0be`; its live binary pair matched that artifact, both intended UDP ports listened, and fresh schema 7 telemetry was growing.
...
- The complete stopped/flushed `b094fd87` capture later found five same-bot/same-door episodes of at least five seconds and a 16.6-second maximum with 41.3% nearly stationary frames, so forward pressure alone was insufficient. Commit `1c1ed0be` adds no route owner: after 350 ms of persistent door contact it holds forward plus one clearance-probed panel-edge tangent, retaining that side across brief recontact. Its first two `obj_team2` sessions had 59 episodes, none at least five seconds; the 4.4-second maximum was moving rather than stuck. Longer multi-map acceptance remains pending.
