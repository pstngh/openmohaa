...
- Deployed commit `5940d700` keeps wall placement separate. Fully-open-panel contact retains its probed tangent only while touching the panel, then carries the captured through-door approach until 128 units of progress or a three-second maximum. A grounded ladder reattachment after the original 64-unit owner releases is rejected and reclaims the established away direction until 128 units or 2.5 seconds. Neither phase repaths or changes ordinary route, combat, lean, crosshair, objective, or wall-comfort ownership. All local tests, server-only run `31517765398`, exact two-file ELF64 x86-64 verification, deployment hashes, service health, ports, cleanup, and fresh schema 7 recording pass; live acceptance remains pending.
...
Accumulate a representative fresh capture on deployed commit `5940d700`, then pull it transiently and compare fully-open-panel completion/timeout and long-contact episodes plus grounded ladder reacquisition timing/exact-origin cycles against `849be809`. Check ordinary progress, objectives, and regressions as controls, delete the pulled copy after analysis, and keep wall placement as a separate owner-authorized experiment.
