...
Owner-test deployed commit `5d3fd2b2` on `obj_team2`, especially right/free-edge door pushes, wall-lane clearance, loop efficiency, teammate stops, and lean transitions. Preserve and pull that clean capture, compare it with the complete `6e472321` baseline, then recheck `obj_team4` and one FFA/practice map before acceptance.
