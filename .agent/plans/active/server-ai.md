...
- Reject deployed experiment `bd5e7f17`. A fresh 0.653-session-hour capture found 39 recovery starts, zero completions, 16 timeouts, and 23 silent external cancellations. All 39 destinations projected exactly -128 units onto the intended away vector because game yaw was applied directly to Recast X/Z axes despite the `(x,y,z)` to `(x,z,-y)` conversion. Fast reattachment and exact-origin clustering did not improve. The transient pull was deleted; the service remains healthy but still runs the rejected pair until replacement deployment.
- Rebuild the experiment from parent `e7476659` as one replacement commit. Preserve the door phase, derive `FindPathAway` angles from the converted Recast preferred vector, cover all four game-space cardinal directions, report cancellation owner reasons, and expose route-recovery activity in frame telemetry. The rejected source and checkpoint commits must not remain in canonical branch history.
...
Commit the replacement on `e7476659`, pass all local suites and continuity, force-push-with-lease the rewritten branch, then use only its exact passing Linux x64 dedicated-server artifact for deployment. Restart through the existing cleanup hook, verify hashes/identity/service/ports/telemetry, and gather a clean behavioral capture before acceptance.
