...
Commit and push the focused free-edge/commitment experiment, require a passing server-only Linux x64 build with exactly `omohaaded` and `game.so`, preserve the current `6e472321` telemetry and binary pair, deploy the verified artifact, and start a clean capture. Compare the same door, wall-lane, loop-efficiency, teammate-stop, and lean-transition measures on `obj_team2`, then recheck `obj_team4` and one FFA/practice map before acceptance.
