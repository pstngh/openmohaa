...
Push the focused geometric door/ladder experiment, require passing server-only Linux x64 CI and an independently verified exact artifact, then revalidate and deploy to the tuning service. The restart must clear telemetry without backing it up; verify runtime identity and collect a fresh comparison before judging either behavior. Keep wall placement as the next separate experiment.
